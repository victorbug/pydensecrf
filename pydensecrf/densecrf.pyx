...
cdef class DenseCRF2D(DenseCRF):
    ...
    def addPairwiseGaussian(self, unsigned char[:,::1] npykerx not None, unsigned char[:,::1] npykery not None, sxy, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy)

        self._this2d.addPairwiseGaussian(&npykerx[0,0], &npykery[0,0], sxy[0], sxy[1], _labelcomp(compat), kernel, normalization)

    def addPairwiseBilateral(self, unsigned char[:,::1] npykerx not None, unsigned char[:,::1] npykery not None, sxy, srgb, unsigned char[:,:,::1] rgbim not None, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy)
        ...
        self._this2d.addPairwiseBilateral(
            &npykerx[0,0], &npykery[0,0], sxy[0], sxy[1], srgb[0], srgb[1], srgb[2], &rgbim[0,0,0], _labelcomp(compat), kernel, normalization
        )
